...
int make_disk(char *name) {
    ...
    if (!name) {
        fprintf(stderr, "make_disk: invalid tufs_file name\n");
        return -1;
    }

    if ((f = open(name, O_WRONLY | O_CREAT | O_TRUNC, 0644)) < 0) {
        perror("make_disk: cannot open tufs_file");
        return -1;
    }
...
int open_disk(char *name) {
    ...
    if (!name) {
        fprintf(stderr, "open_disk: invalid tufs_file name\n");
        return -1;
    }
    ...
    if ((f = open(name, O_RDWR, 0644)) < 0) {
        perror("open_disk: cannot open tufs_file");
        return -1;
    }
